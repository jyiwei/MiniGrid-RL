...
class BaseRIDEAlgo(ABC):
    ...
    def __init__(self, envs, acmodel, icm, device, num_frames_per_proc, discount, lr, gae_lambda, entropy_coef,
                 value_loss_coef, intrinsic_reward_weight, fwd_loss_weight, icm_policy_weight, max_grad_norm, 
        ...
        intrinsic_reward_weight: float
            the relative weight given to the instrinsic reward with respect to the environment reward
            i.e. the eta in total_reward = extrinsic_reward + eta * intrinsic_reward
        fwd_loss_weight: float
            the relative weight given to the forward loss with respect to the backward loss
            i.e. the beta in icm_loss = beta * fwd_loss + (1 - beta) * inv_loss
        recurrence : int
        ...
        self.fwd_loss_weight = fwd_loss_weight
        ...
        # Configure intrinsic curiosity module (ICM)
        ...
        self.log_episode_intrinsic_return = torch.zeros(self.num_procs, device=self.device)
        self.log_episode_num_frames = torch.zeros(self.num_procs, device=self.device)

        self.log_done_counter = 0
        self.log_return = [0] * self.num_procs
        self.log_reshaped_return = [0] * self.num_procs
        self.log_intrinsic_return = [0] * self.num_procs
    ...
    def collect_experiences(self):
        ...
        for i in range(self.num_frames_per_proc):
            ...
            self.log_episode_return += extrinsic_reward
            self.log_episode_reshaped_return += extrinsic_reward
            self.log_episode_intrinsic_return += intrinsic_reward
            ...
            for i, done_ in enumerate(done):
                if done_:
                    ...
                    self.log_intrinsic_return.append(self.log_episode_intrinsic_return[i].item())
                    self.log_reshaped_return.append(self.log_episode_reshaped_return[i].item())
                    self.log_num_frames.append(self.log_episode_num_frames[i].item())

            self.log_episode_return *= self.mask
            self.log_episode_reshaped_return *= self.mask
            self.log_episode_intrinsic_return *= self.mask
        ...
        logs = {
            ...
            "intrinsic_return_per_episode": self.log_intrinsic_return[-keep:],
        ...
        self.log_intrinsic_return = self.log_intrinsic_return[-self.num_procs:]
