...
class PPO_RIDE_Algo(BaseRIDEAlgo):
    ...
    def __init__(self, envs, acmodel, icm, device=None, num_frames_per_proc=None, discount=0.99, lr=0.001, gae_lambda=0.95,
                 entropy_coef=0.01, value_loss_coef=0.5, intrinsic_reward_weight=0.01, fwd_loss_weight=0.2, icm_policy_weight=0.1,
                 max_grad_norm=0.5, recurrence=4, adam_eps=1e-8, clip_eps=0.2, epochs=4, batch_size=256, preprocess_obss=None,
                 reshape_reward=None):
    ...
    def update_parameters(self, exps):
        ...
        for _ in range(self.epochs):
            ...
            for inds in self._get_batches_starting_indexes():
                ...
                for i in range(self.recurrence):
                    ...
                    # curiosity loss
                    one_hot_action = F.one_hot(sb.action.to(torch.int64), num_classes=7).float().detach()
                    # Calculate inverse and forward loss
                    # print(sb.obs.image.shape)
                ...
                grad_norm = sum(p.grad.data.norm(2).item() ** 2 for p in list(self.acmodel.parameters()) + list(self.icm.parameters())) ** 0.5
                torch.nn.utils.clip_grad_norm_(list(self.acmodel.parameters()) + list(self.icm.parameters()), self.max_grad_norm)
        ...
        logs = {
            ...
            "inverse_loss": numpy.mean(log_backward_loss),
            "forward_loss": numpy.mean(log_forward_loss),
            "curiosity_loss": numpy.mean(log_value_losses),
